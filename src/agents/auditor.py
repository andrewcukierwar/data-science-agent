...
2. For every table, record schema/types, row count, date coverage, missingness,
   candidate keys, duplicate rates, and plausible relationships.
3. Check expected temporal granularity and internal gaps. Distinguish a missing
   reporting day from a valid sparse event table.
4. Check duplicates only for candidate identifiers; repeated foreign-key values
   are not duplicate records by themselves.
5. Report only anomalies supported by executed SQL/Python evidence. Prefer a
...
- Enumerate every available input table/file and inspect its schema and types.
...
- Check obvious anomalies with bounded SQL or reproducible Python, including
  ...
  the data supports the comparison.
